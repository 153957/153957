...
var ext = ".mov";
var type = "mp4";
var html5 = false; /* No HTML5 -> Use QuicktTime Embed */
...
$(document).ready(function () {
    ...
    $('#categories').on('click', ".category", function () {
        ...
        if ($('#' + numm).get(0) !== $('#thumbnails').find('div.currentslide').get(0)) {
            $('#thumbnails').find('div.currentslide').animate({height: "hide", opacity: "hide"}, "fast").removeClass("currentslide").addClass("slidehide");
            $('#' + numm).animate({height: "show", opacity: "show"}, "fast").removeClass("slidehide").addClass("currentslide");
    ...
    $('#thumbnails').on('click', ".thu", function () {
        $glo.thu.removeClass('currentmovie')
...
function playingpaused() {
    if ($glo.vid.paused) {$con.ppb.addClass('fa-play').removeClass('fa-pause');}
    else                 {$con.ppb.removeClass('fa-play').addClass('fa-pause');}}
...
function volumeused() {
    ...
    if (percentUsed == 0) {
        $con.vsb.removeClass('fa-volume-up').removeClass('fa-volume-down').addClass('fa-volume-off');}
    else if (percentUsed <= 40) {
        $con.vsb.removeClass('fa-volume-up').addClass('fa-volume-down').removeClass('fa-volume-off');}
    else {
        $con.vsb.addClass('fa-volume-up').removeClass('fa-volume-down').removeClass('fa-volume-off');}}
...
function setHWM() {
    ...
    $glo.vid.style.margin = '0 0 0 ' + hmargin + 'px';
 ...
 /* Attatch Webkit Controls */
    if (ctrl && kind !== "ipod" && !ctrls) {
        ...
        if (sf) {
            ...
            var full = '<div id="fullscreen_button" class="fa fa-expand" onclick="fullscreen()"></div>';}
        else {
        ...
        $glo.mov.append(
            '<div id="controls" style="margin-left:' + hmargin + 'px; width:' + $glo.vid.width + 'px;">\
                 ...
                 <div id="progress_bar">\
                     ...
                     <input id="progress_indicator" type="range" step="any" value="0" min="0" max="' + $glo.vid.duration + '"\
...
function errorStatus() {
    $glo.sta.removeClass('fa-spinner fa-spin hidden')
            .addClass('fa-times-circle-o showing');}
...
function swapVideo(movieid) {
    ...
    if (html5) {
        if (kind === 'ipod') {
            ...
            $glo.mov.html('<span id="status" class="fa fa-spinner fa-spin showing"></span><video src=' + videolink +
                          ' id="player" style="margin: 0px 0px;" height="0" width="0" preload="auto" onloadedmetadata="setHWM()" oncanplay="hideLoad()"></video>');}
        $glo.vid = $('#player').get(0);
        $glo.sta = $('#status');
        $glo.vid.addEventListener('error', errorStatus, true);}
